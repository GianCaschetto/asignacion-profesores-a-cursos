...
function App() {
  ...
  const cambiarPreferencia = (curso, profesor, valor) => {
    ...
    nuevasPreferencias[curso][profesores.indexOf(profesor)] = parseInt(valor);
    ...
    const explorarAsignaciones = (cursoIndex) => {
      if (cursoIndex === cursos.length) {
        ...
        cursos.forEach((curso) => {
          ...
          const puntuacion =
            preferencias[curso][profesores.indexOf(profesorAsignado)];
    ...
    <div className="container mx-auto mt-5 p-5 content-center">
      ...
      <div className="w-full max-w-xl mx-auto">
            ...
            <tbody>
              {cursos.map((curso, cursoIndex) => (
                ...
                >
                  ...
                  {profesores.map((profesor, profesorIndex) => (
                    <td key={profesorIndex} className="px-6 py-4  text-white">
                      {edicionPreferencias ? (
                        <input
                          type="number"
                          className="w-10 h-8 text-center border rounded-lg text-black"
                          value={preferencias[curso][profesorIndex]}
                          onChange={(e) =>
                            cambiarPreferencia(curso, profesor, e.target.value)
                          }
                        />
                      ) : (
                        preferencias[curso][profesorIndex]
                      )}
                    </td>
                  ))}
                </tr>
        ...
        <div className="flex justify-center mb-5">
          <button
            onClick={() => setEdicionPreferencias(!edicionPreferencias)}
